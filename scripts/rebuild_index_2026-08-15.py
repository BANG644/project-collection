#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-15 索引同步：以磁盘 349 份报告为权威源，追加 5 个 Trending 新仓库
（rustdesk/rustdesk、ToolJet/ToolJet、citrolabs/ego-lite、
lightningpixel/modly、deepseek-ai/awesome-deepseek-agent），计数 344→349。
复用 2026-08-14 磁盘权威重建逻辑。STALE = 空（无删除/改名）。
"""
...
CURATED = {
 "rustdesk/rustdesk": dict(repo="rustdesk/rustdesk", file="rustdesk-rustdesk-深度调研.md",
    desc="用 Rust 重写、AGPL-3.0 的开源远程桌面（TeamViewer/AnyDesk 自托管替代）——Rust 核心+Flutter UI、P2P 直连优先+中继可自建、全链路自研编解码/捕获/输入库",
    stars="120,559", date="2026-08-15", domain="安全 / 运维 / 系统工具"),
 "ToolJet/ToolJet": dict(repo="ToolJet/ToolJet", file="ToolJet-ToolJet-深度调研.md",
    desc="AGPL-3.0 开源企业内部工具/低代码平台——拖拽式 App Builder+80+ 数据源+内置 DB，AI 原生（自然语言生成应用/Agent Builder），Retool 开源替代，NestJS+React 全栈",
    stars="38,963", date="2026-08-15", domain="前端 / 设计 / UI"),
 "citrolabs/ego-lite": dict(repo="citrolabs/ego-lite", file="citrolabs-ego-lite-深度调研.md",
    desc="从零为「人和 AI Agent 共用浏览器」设计的 Chromium 浏览器——Agent 在隔离 Space 并行跑任务、继承用户登录态，代码接口（JS 一次性执行）非 CLI，MIT 连接层",
    stars="10,261", date="2026-08-15", domain="AI Agent / 助手框架"),
 "lightningpixel/modly": dict(repo="lightningpixel/modly", file="lightningpixel-modly-深度调研.md",
    desc="完全本地、MIT 开源的 AI 图像转 3D 网格桌面应用——消费级 GPU 跑 Hunyuan3D 等模型、扩展式模型架构、导出 GLB/OBJ/STL/PLY，含 Agent CLI+MCP，无云无账号",
    stars="5,857", date="2026-08-15", domain="视频 / 音频 / 多媒体"),
 "deepseek-ai/awesome-deepseek-agent": dict(repo="deepseek-ai/awesome-deepseek-agent", file="deepseek-ai-awesome-deepseek-agent-深度调研.md",
    desc="DeepSeek 官方维护的精选清单——覆盖 24+ 主流 Agent/编程工具接入 DeepSeek-V4 的中英双语可执行指南（安装/配置/首跑），awesome list 的「runbook 化」进化",
    stars="5,640", date="2026-08-15", domain="学习资料 / 方法论"),
}
