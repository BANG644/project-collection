#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-26 索引同步：以磁盘 386 份报告为权威源。
本批 = 5 份新增（freestylefly/awesome-gpt-image-2 / apache/maka / marin-community/marin / asciimoo/hister / anthropics/claude-plugins-community）。
计数 381→386。复用 2026-08-25 磁盘权威重建逻辑。STALE = 空（无删除/改名）。
"""
...
CURATED = {
 "freestylefly/awesome-gpt-image-2": dict(repo="freestylefly/awesome-gpt-image-2", file="freestylefly-awesome-gpt-image-2-深度调研.md",
    desc="GPT-Image-2「提示词即代码」工业级引擎：532 逆向案例 + 原子化 style-library schema + 可安装 Agent Skill + 网页画廊，MIT",
    stars="17,408", date="2026-08-26", domain="其他"),
 "apache/maka": dict(repo="apache/maka", file="apache-maka-深度调研.md",
    desc="Apache 孵化本地优先 AI Agent 工作区，事件溯源 append-only 日志记录执行事实，Runtime Host 单一权威 + eval 原生，Apache-2.0",
    stars="3,254", date="2026-08-26", domain="AI Agent / 助手框架"),
 "marin-community/marin": dict(repo="marin-community/marin", file="marin-community-marin-深度调研.md",
    desc="Open Athena 开源基础模型研发框架，数据→预训→后训→评测全链路，开放开发哲学 + Delphi scaling suite，Apache-2.0",
    stars="2,049", date="2026-08-26", domain="其他"),
 "asciimoo/hister": dict(repo="asciimoo/hister", file="asciimoo-hister-深度调研.md",
    desc="searxng 作者出品的私人全文搜索引擎，索引浏览历史与本地文件，隐私优先 + MCP 桥接 AI 助手，AGPL-3.0",
    stars="2,692", date="2026-08-26", domain="其他"),
 "anthropics/claude-plugins-community": dict(repo="anthropics/claude-plugins-community", file="anthropics-claude-plugins-community-深度调研.md",
    desc="Anthropic 官方社区插件市场（只读镜像），所有插件经自动安全扫描+审批，claude plugin 一键安装，Apache-2.0",
    stars="1,660", date="2026-08-26", domain="AI 编码 / Skill 技能"),
}
CURATED_N = {norm(v["repo"]): v for k, v in CURATED.items()}

# ============ 1) 主表重建（磁盘权威）============
lines = io.open(IDX, encoding="utf-8").read().split("\n")
sep_i = next(i for i, ln in enumerate(lines) if True and re.match(r"^\|[\-:\s|]+\|$", ln))
end_i = sep_i + 1
