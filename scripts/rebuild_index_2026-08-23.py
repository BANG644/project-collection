#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-23 索引同步：以磁盘 376 份报告为权威源。
本批 = 5 份新增（openai/codex / anthropics/claude-code / multica-ai/andrej-karpathy-skills / microsoft/TypeScript / ripienaar/free-for-dev）。
计数 371→376。复用 2026-08-22 磁盘权威重建逻辑。STALE = 空（无删除/改名）。
"""
...
CURATED = {
 "openai/codex": dict(repo="openai/codex", file="openai-codex-深度调研.md",
    desc="OpenAI 终端编码 Agent（Codex CLI）——Rust 核心 + OS 级沙箱优先 + Goal 模式长程自主 + 云端异步开 PR，Apache-2.0 开源可审计",
    stars="112,897", date="2026-08-23", domain="AI Agent / 助手框架"),
 "anthropics/claude-code": dict(repo="anthropics/claude-code", file="anthropics-claude-code-深度调研.md",
    desc="Anthropic 终端 Agentic 编码工具——approval-gated loop + subagents/hooks/SKILL.md 编排 + 最成熟 MCP 生态（无 SPDX 闭源分发）",
    stars="142,461", date="2026-08-23", domain="AI Agent / 助手框架"),
 "multica-ai/andrej-karpathy-skills": dict(repo="multica-ai/andrej-karpathy-skills", file="multica-ai-andrej-karpathy-skills-深度调研.md",
    desc="Karpathy 把 LLM 编码常见坑浓缩成单文件 CLAUDE.md 行为准则（谨慎优先），跨 harness 即插即用，205K⭐ 现象级提示词项目",
    stars="205,207", date="2026-08-23", domain="AI Agent / 助手框架"),
 "microsoft/TypeScript": dict(repo="microsoft/TypeScript", file="microsoft-TypeScript-深度调研.md",
    desc="JavaScript 超集类型语言——7.0 用 Go 重写编译器（Project Corsa/tsgo）获 8–12× 提速且语义零变化，双轨过渡",
    stars="110,501", date="2026-08-23", domain="开发工具 / CLI"),
 "ripienaar/free-for-dev": dict(repo="ripienaar/free-for-dev", file="ripienaar-free-for-dev-深度调研.md",
    desc="面向 DevOps 的免费 tier 服务清单——47 类精细 taxonomy + 严格收录红线 + 1600+ 人 PR 维护，开发者白嫖基础设施入口",
    stars="133,804", date="2026-08-23", domain="其他"),
}
