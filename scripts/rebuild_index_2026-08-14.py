#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-14 索引同步：以磁盘 344 份报告为权威源，追加 5 个 Trending 新仓库
（unslothai/unsloth、kepano/obsidian-skills、megadose/holehe、
holaboss-ai/holaOS、cactus-compute/needle），计数 339→344。
复用 2026-08-10 磁盘权威重建逻辑。STALE = 空（无删除/改名）。
"""
...
CURATED = {
 "unslothai/unsloth": dict(repo="unslothai/unsloth", file="unslothai-unsloth-深度调研.md",
    desc="本地运行+微调 LLM/扩散模型的桌面与库一体方案——手写 GPU kernel 实现 2× 更快、70% 更省显存，支持 LoRA/QLoRA/RL，可接 Agent 后端，Apache-2.0",
    stars="70,961", date="2026-08-14", domain="LLM / 推理框架"),
 "kepano/obsidian-skills": dict(repo="kepano/obsidian-skills", file="kepano-obsidian-skills-深度调研.md",
    desc="遵循 Agent Skills 规范的 Obsidian 技能包——覆盖 MD/Bases/JSON Canvas/CLI，Defuddle 网页去噪提取省 token，MIT",
    stars="45,579", date="2026-08-14", domain="AI 编码 / Skill 技能"),
 "megadose/holehe": dict(repo="megadose/holehe", file="megadose-holehe-深度调研.md",
    desc="OSINT 邮箱侦查工具——借「忘记密码」等接口被动判断某邮箱是否在 120+ 站点注册、不告警，Trio 异步、GPL-3.0",
    stars="12,332", date="2026-08-14", domain="安全 / 运维 / 系统工具"),
 "holaboss-ai/holaOS": dict(repo="holaboss-ai/holaOS", file="holaboss-ai-holaOS-深度调研.md",
    desc="开源一体化 AI Agent 工作区——任意 Agent 同本地优先工作区共享记忆/工具，HolaApps 真实 UI 并排，100+ 集成+MCP，Modified Apache 2.0",
    stars="6,447", date="2026-08-14", domain="AI Agent / 助手框架"),
 "cactus-compute/needle": dict(repo="cactus-compute/needle", file="cactus-compute-needle-深度调研.md",
    desc="14MB 单文件端侧基础模型（Needle 2, 45M）——字节级语法约束解码+置信度门控+工具检索，离线设备端工具调用/结构化抽取，MIT",
    stars="4,830", date="2026-08-14", domain="LLM / 推理框架"),
}
...
rd = rd.replace(f"| [🎯 全量规整后总览]", f"| [🎯 全量规整后总览]")
rd = rd.replace("| 339 |", f"| {TOTAL} |")   # 总览 / 索引表 行
