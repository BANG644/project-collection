#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-31 索引同步：以磁盘 398 份报告为权威源。
本批 = 3 份新增（p-e-w/heretic / THU-MAIC/OpenMAIC / colinhacks/zod）+ 2 份重写校正
（makeplane/plane / modular/modular 修正星标与许可）。计数 395→398。
复用 2026-08-30 磁盘权威重建逻辑。
"""
...
CURATED = {
 "p-e-w/heretic": dict(repo="p-e-w/heretic", file="p-e-w-heretic-深度调研.md",
    desc="用 abliteration（方向性消融）把对齐模型变成「异端」——rank-1 LoRA 增量 + Optuna TPE 多目标优化拒答率/KL，AGPL-3.0，+150/日",
    stars="29,063", date="2026-08-31", domain="LLM / 推理框架"),
 "THU-MAIC/OpenMAIC": dict(repo="THU-MAIC/OpenMAIC", file="THU-MAIC-OpenMAIC-深度调研.md",
    desc="开源 AI 幻灯片/视频生成引擎——Next.js 16 + LangGraph 编排导演图，@openmaic SDK 家族，MIT，当日 Trending 增速榜首（+907/日）",
    stars="23,639", date="2026-08-31", domain="视频 / 音频 / 多媒体"),
 "colinhacks/zod": dict(repo="colinhacks/zod", file="colinhacks-zod-深度调研.md",
    desc="TypeScript 类型安全校验库——v4 引入 compile() JIT 编译 + $constructor trait 系统 + ZSF 中立交换格式，MIT，Trending",
    stars="43,626", date="2026-08-31", domain="开发工具 / CLI"),
 "makeplane/plane": dict(repo="makeplane/plane", file="makeplane-plane-深度调研.md",
    desc="开源项目管理和问题追踪平台（Jira 替代品）——6 应用单仓（web/api/space/admin/live/proxy），实时协作靠 apps/live 的 Hocuspocus+Yjs CRDT，AGPL-3.0，默认分支 preview",
    stars="58,558", date="2026-08-31", domain="开发工具 / CLI"),
 "modular/modular": dict(repo="modular/modular", file="modular-modular-深度调研.md",
    desc="统一的 AI 开发部署平台——Mojo 语言（Python 语法+系统级性能）+ MAX 推理框架（serve/kernels/pipelines），Mojo 1.0.0/MAX 26.5，许可 NOASSERTION（MAX 运行时另受 Modular Community License 约束）",
    stars="29,365", date="2026-08-31", domain="开发工具 / CLI"),
}
