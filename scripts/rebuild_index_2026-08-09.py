#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""2026-08-09 索引同步：以磁盘 324 份报告为权威源，追加 4 个 Trending 新仓库
（TauricResearch/TradingAgents、LadybirdBrowser/ladybird、google/guava、goauthentik/authentik），计数 320→324。
复用 2026-08-08 磁盘权威重建逻辑（主表 / 字母组 / 领域分类 / 计数）。STALE = 空（无删除/改名）。
"""
...
CURATED = {
 "tauricresearch/tradingagents": dict(repo="TauricResearch/TradingAgents", file="TauricResearch-TradingAgents-深度调研.md",
    desc="多 Agent LLM 金融交易框架——分析师→多空辩论→交易员→风控→基金经理五层决策链，LangGraph 编排 + 可插拔数据源",
    stars="96,335", date="2026-08-09", domain="AI Agent / 助手框架"),
 "ladybirdbrowser/ladybird": dict(repo="LadybirdBrowser/ladybird", file="LadybirdBrowser-ladybird-深度调研.md",
    desc="真正独立的网页浏览器——从零构建引擎（非 Blink/WebKit/Gecko fork），多进程架构 + 自研 LibWeb/LibJS，非营利 501(c)(3)",
    stars="64,934", date="2026-08-09", domain="其他"),
 "google/guava": dict(repo="google/guava", file="google-guava-深度调研.md",
    desc="Google 核心 Java 工具库——集合/缓存/并发/IO 等基础设施，Java 生态事实标准（Apache-2.0）",
    stars="51,829", date="2026-08-09", domain="开发工具 / CLI"),
 "goauthentik/authentik": dict(repo="goauthentik/authentik", file="goauthentik-authentik-深度调研.md",
    desc="开源身份提供商（IdP）——SSO/OAuth2/OIDC/SAML/LDAP/反向代理，Django+Rust+TS 混合栈，Blueprint GitOps",
    stars="23,912", date="2026-08-09", domain="安全 / 运维 / 系统工具"),
}
