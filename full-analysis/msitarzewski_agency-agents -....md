...
`msitarzewski/agency-agents` 是一个AI agent role library项目：预置多角色 AI agency agents，从前端、社区运营到创意和现实检查等角色集合。

> 核心判断：价值在角色分工和交付模板。但它不能只按 README 口号理解，必须同时看真实源码结构、权限边界、维护节奏和实际任务验证。容易变成 prompt 角色库，必须验证输出质量和边界。
...
| 类型 | AI agent role library |
| 核心价值 | 价值在角色分工和交付模板 |
| 主要风险 | 容易变成 prompt 角色库，必须验证输出质量和边界 |
...
| 方案 | 优势 | 风险 |
|---|---|---|
| msitarzewski/agency-agents | 垂直场景明确，能快速试用 | 需要验证维护质量和真实边界 |
