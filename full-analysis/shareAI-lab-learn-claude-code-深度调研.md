...
> 报告日期：2026-07-06 | **Star 更新：71,335（2026-07-18）** | Fork：11,605 | 主语言：Python | 许可证：MIT
> 核心哲学：**"Bash is all you need"**——从零构建 Agent 运行环境（Harness）
> 📌 2026-07-18 补完：新增「第九章：核心研判」「第十章：关键文件路径速查」
...
| 指标 | 数据 |
|---|---|
| Stars | 71,335（2026-07-18 更新，较初版 70,067 涨约 1.3K；2026 年 3 月发布，3 个多月从 0 暴涨至 7 万+） |
| Forks | 11,415 |
...
## 第九章：核心研判（补完于 2026-07-18）

### 9.1 项目优势

- **教学法行业第一**：先给心智模型再给代码、Ascii 图示贯穿、每课独立可运行 `code.py`、一句 motto 浓缩精髓——把"Agent Harness 工程"讲透，显著降低理解门槛。
- **哲学锋利**：「模型即 Agent」一针见血，对"Agent 框架工业"的批判有说服力，帮工程师建立正确心智模型。
- **生态闭环**：教学 + Kode CLI（生产级）+ Kode Agent SDK + Skills 四位一体，学完能直接落地。
- **多语言 + 开放**：中/英/日三语 README，MIT 协议，完全免费。

### 9.2 项目风险

- **教学代码不可直接生产**：权限 / 会话 / MCP 均为教学级简化，直接上线必踩坑（已有多人中招）。
- **入口体验缺陷**：官网 Quiz 评分逻辑 Bug（全选 D 反而判 Beginner）直接劝退 HN 上的潜在读者。
- **版本碎片化**：旧 12 课（`agents/` + `docs/`）与新 20 课（`s01-s20`）共存，`web/` 渲染内容落后于根目录。
- **多模型隐性成本**：教程代码与说明完全围绕 Anthropic API，切换到 GLM/DeepSeek/Kimi 后工具调用行为、token 计数、错误处理等差异未覆盖。

### 9.3 趋势判断

- **Agent Harness 教学会成为 AI 工程教育的新刚需**，learn-claude-code 已占先发位，先发优势明显。
- 价值重心在「懂原理」而非「有工具」——这与模型能力快速进步互补：框架热退潮、原理热升温。
- 配套 **Kode CLI** 若把多模型体验打磨好，可承接「学完即生产」的转化，形成教学→产品的飞轮。

### 9.4 给同类需求的启发

- **想理解 / 自研 Agent**：先读它建立心智，再去看 Claude Code / OpenCode / Pi 的源码，事半功倍。
- **做技术教学**：用「渐进式单循环 + 每课加一层」的结构，比一次性甩完整框架更易吸收——这是它最值得借鉴的方法论本身。
...
| 路径 | 作用 |
|------|------|
| `s01_agent_loop/code.py` | 核心循环（约 60 行），整座 Harness 的基石 |
| `agents/s_full.py` | 20 课机制的完整参考实现（741 行、18 函数/类，按 `SECTION` 注释组织） |
| `s03_permission/code.py` | 权限管道（教学级 Gate，已知 Windows `del` 绕过问题 #448） |
| `s08_compaction/code.py` | 上下文压缩（reactive_compact） |
| `s19_mcp/code.py` | MCP 插件接入（缺 transport / OAuth 处理） |
| `skills/` | agent-builder / code-review / pdf / mcp-builder 四个 Skill 文件 |
| `web/` | Next.js 网页教学平台（内容落后于根目录章节，见 #441） |
| `requirements.txt` | 仅 3 个依赖：`anthropic` / `python-dotenv` / `pyyaml` |
